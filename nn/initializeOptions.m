function opt = initializeOptions()
	...
	opt.batchSize = 50;
	opt.numEpochs = 10;
	opt.input_do_rate = 0.8; % Probability to keep the input node in active (not dropout rate)
	opt.hidden_do_rate = 0.5; % Probability to keep the hidden node in active (not dropout rate)
	opt.gaussian = false; % Multiply Gaussian Noise to Input data
	opt.dropout = true;
	opt.adaptive = false;
    opt.sobol = false;
    opt.halton = false;
