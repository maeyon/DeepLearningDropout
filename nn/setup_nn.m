function net = setup_nn(net, x , y, opt)
	...
	for l = 2:length(net.layers)-1
		net.layers{l}.w = 0.1*(rand(net.layers{l}.n, numNodePrev) * 2 - 1); % Initialize weight matrix - value between -0.1 and 0.1
	...
	l = length(net.layers);
	net.layers{l}.w = 0.1*(rand(size(y,1),numNodePrev) * 2 - 1); % Initialize weight matrix - need to experiment with different methods
