function output = my_feedForward_test_dropout_nn(net, x)
    numLayers = length(net.layers); % total number of layers
    net.layers{1}.a = x;
    
    ido = sig(net.layers{1}.lambda);
    for l = 2:length(net.layers)-1
        hdo{l} = sig(net.layers{l}.lambda);
    end
	...
	for l = 2 : numLayers
        ...
        if l < numLayers
            net.layers{l}.do = rand(size(net.layers{l}.a)) <= hdo{l};
